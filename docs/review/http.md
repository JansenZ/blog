...
3. POST 与 GET 的区别
   ...
   - GET 比 POST 更不安全，因为参数直接暴露在 URL 上，所以不能用来传递敏感信息。所谓安全定义是给普通人看的，如果想要更安全，post 一会被知道发送的啥。
...
25. XSS、CSRF（如何攻击，如何防御）

    XSS 攻击，跨站脚本攻击，cross site scripting

    分为几类，一个是存储型攻击，比如在评论区发执行脚本，那么这段脚本就被存储起来了，其他用户打开就会执行，如果他写了个盗取你的 cookie 的，你就没了

    一个是反射型攻击，比如在 url 上的某参数上写脚本，然后如果前端没处理直接显示的话，就会执行该脚本
