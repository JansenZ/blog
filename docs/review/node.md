...
2. nodejs 写习惯后，回调函数第一个参数代表是否有错误，没有就 `callback(null,’ success’)`
3. RPC 调用和 AJAX 的异同点
    1. RPC 是服务端到服务端的调用，
    2. AJAX 是浏览器到服务端的调用
    3. 都是两个计算机之间的网络通信
    ...
    6. RPC 应用层协议一半不使用 HTTP，都是二进制协议,更小的数据包体积，更快的编解码速率。
    7. RPC 基于 TCP 或 UDP 协议。
4. nodemon 是什么？
   nodemon 可以自己监控 nodejs 文件的变化，然后重启服务。不用自己重新输入命令了, nodemen xxx.js 既可。
5. TCP 分为单工通信，半双工通信，全双工通信， 怎么理解？
...
6. 双工通信需要解决什么问题
    1. 标记包号，防止返回顺序时间不定
    2. 粘包的情况，需要结合旧包，自己分割
7. Koa 比 Express 的优势在于，express 的中间件不支持异步，导致洋葱模型被破坏，而Koa可以。
```js
