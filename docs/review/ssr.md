...
3. 描述一下 React 的 SSR 全流程
   ...
   2. 服务端把数据注入到 window.initialData 中
   ...
   8. React，使用 react-dom/server 提供的 renderToString 或 renderToPipeableStream（流失渲染） 方法。
   9. Vue，使用 vue-server-renderer 提供的 renderToString 方法。流式渲染需要额外的插件支持。
   10. 客户端接管的时候，React 用的是 hydrate，Vue 用的是 mount
   11. 事件绑定的时候 React 是用的合成事件，Vue 是原生事件
...
9. 完整的描述下 react ssr 全过程。

1. 客户端访问 url
1. url 打到 node 服务器，node 拿到 url 后，路由解析 path，找到对应的路由组件。
1. 然后调用组件下的 getdata 方法，服务器接收到请求后返回数据
1. node 把这个数据放到 StaticRouter 的 context 属性下，这样组件里就可以通过 props.staticContext 获取对应数据。
