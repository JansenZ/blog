...
37. NPM install 运行机制
    ...
    1. 它会先遍历的访问所有的依赖，包括依赖里的依赖，然后，把他们存到根节点下，相当于一个Map的概念，这样有重复的模块就不用再添加了

        而重复的模块的定义就是名称一样，和版本兼容一样。（就是范围允许的值）

    2. 如果你npm install 具体某个包名，同样会去检查package.json。保证前后的一致性。
