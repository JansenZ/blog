...
30. **V8的内存回收机制是怎么回事？**  
    ...
    V8 引擎采用 分代回收（Generational Garbage Collection） 机制来管理内存。该机制根据对象的生命周期来分配内存和进行回收。V8 将内存划分为两个主要区域：新生代（Young Generation） 和 老生代（Old Generation）。

    - 新生代（Young Generation）特点：
        1. 存储生命周期较短的对象。大部分新创建的对象都位于新生代。
        2. 新生代回收频率较高，因为很多对象在创建后很快就会被销毁。
        3. 超出分配的阈值后就会触发，正常是2-16MB

    - 新生代回收方式：
      * 新生代的垃圾回收采用 复制算法（Copying Algorithm）。
      * 新生代内存被划分为两个区域：From Space 和 To Space。
      * 在回收过程中，**存活**的对象会从 From Space 复制到 To Space 中，清理掉不再使用的对象。
      * 回收后， From Space 会被清空， To Space 会成为新的活动区域。

    - 老生代（Old Generation）特点：
        1. 存储生命周期较长的对象（例如，长时间存活的对象或者大的数据结构）。
        2. 一旦对象在新生代存活了足够长的时间，它们会被晋升到老生代。
        3. 超出分配阈值或者是有新生代晋升就会触发，正常是200MB-2GB

    - 老生代回收方式：
      * V8 在老生代采用 标记-清除（Mark-and-Sweep）算法来进行垃圾回收。
      * 在标记阶段，V8 会扫描所有对象，标记出存活的对象。
      * 在清除阶段，V8 会清理那些没有被标记的对象，释放其占用的内存。

    - 新生代晋升方式：
      1. 如果每次gc都扫不死它，存活2次以后，它就会晋升。
      2. 如果从From区复制到To区比如To区存活的对象直接就占满空间的25%以上（或者是分配的阈值），就直接晋升。
