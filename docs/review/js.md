...
4. weakmap 用过吗？ 知道它的使用场景吗？
   ...
   `weakmap`就是弱引用，这样对于`gc`会更友好，而且只支持对象，使用场景的话，比如写一个偏向公共的类
...
20. foreach 和 map 的区别
    ...
    foreach 是遍历数组中的元素，没有返回值，通常需要修改原始数组的时候可以用 foreach
...
39. 0.1+0.2 为什么不等于 0.3
    ...
    因为 JavaScript 使用的是 64 位双精度浮点数编码，所以它的符号位占 1 位，指数位占 11 位，尾数位占 52 位。
...
46. JavaScript 可以存储的最大数字、最大安全数字，JavaScript 处理大数字的方法、避免精度丢失的方法
    ...
    - Number.MAX_VALUE 可存储的最大数字 == (Math.pow(2,53) - 1) \* Math.pow(2, 971) 。 971 = 1023 - 52；
